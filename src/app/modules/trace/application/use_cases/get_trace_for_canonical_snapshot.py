...
class GetTraceForCanonicalSnapshotUseCase:
    def __init__(self, uow: TraceUoWPort) -> None:
        self.uow = uow

    @async_transactional(read_only=True)
    async def execute(
        self, query: GetTraceForCanonicalSnapshotQuery
    ) -> list[TraceRecordDTO]:
        rows = await self.uow.trace_raw_to_canonical.get_by_canonical_snapshot(
