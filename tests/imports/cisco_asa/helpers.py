...
FIXTURES_DIR = Path(__file__).resolve().parents[2] / "fixtures" / "cisco_asa"
...
def traces_on_line(
    ...
    return [
        r
        for r in records
        if r.source_line_start <= line_no <= r.source_line_end
    ]
...
def assert_issue(
    ...
    *,
    ...
    if entity_key_substr is not None:
        matches = [i for i in matches if entity_key_substr in i.entity_key]
